...
import digilent_nexys4ddr_platform
...
# class my_dma(Module, AutoCSR):
#     def __init__(self, sdram_port):  # Passer sdram_port en paramètre
#         self.bus = wishbone.Interface()
        
#         self.submodules.reader = WishboneDMAReader(bus=self.bus, with_csr=True)
#         self.submodules.writer = LiteDRAMDMAWriter(sdram_port, with_csr=True)
...
class BaseSoC(SoCCore):
    def __init__(self, sys_clk_freq=75e6,
        ...
        **kwargs):
        platform = digilent_nexys4ddr_platform.Platform()
        ...
        if not self.integrated_main_ram_size:
            ...
            # sdram_port = self.sdram.crossbar.get_port()

            # self.submodules.my_dma = my_dma(sdram_port)
            # self.bus.add_master(name="my_dma", master=self.my_dma.bus)

            # # Backbuffer ---------------------------------------------------------------------------
            # backbuffer_base = self.mem_map.get("video_backbuffer", None)
            # if backbuffer_base is None:
            #     self.bus.add_region("video_backbuffer", SoCRegion(
            #         origin=0x41400000,  # Définir une adresse mémoire différente
            #         size=0x800000,      # Taille de la mémoire du backbuffer
            #         linker=True)
            #     )
            #     backbuffer_base = self.bus.regions["video_backbuffer"].origin

            # self.add_csr("backbuffer_base")
